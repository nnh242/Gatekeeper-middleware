...
//  1. looks for a 'x-username-and-password' request header
  const requestedHeader= queryString.parse(req.get('x-username-and-password'));
//2. parses values sent for `user` and `pass` from 'x-username-and-password'
  const user = requestedHeader.user || null;
  const pass = requestedHeader.pass || null;
// 3. looks for a user object matching the sent username and password values
//4. if matching user found, add the user object to the request object
//(aka, `req.user = matchedUser`)
// i don't fully understand this part of the solution specifically the part where
// i have to pass the input AND the index into this find function, and why I don't have
// to use const or let here to assign req.user to the object 
  req.user = USERS.find((instance,index) => instance.userName === user && instance.password === pass);
  next();
}
